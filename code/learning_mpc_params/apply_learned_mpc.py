...
시연 데이터로부터 KKT 조건 완화를 이용하여 MPC 비용함수 가중치를 학습합니다.

실행 순서:
    1. python main.py  (시연 데이터 생성)
    2. python learn_mpc_weights.py  (이 스크립트 - 가중치 학습)
    3. python apply_learned_mpc.py  (학습된 가중치로 MPC 실행)
"""
...
def main():
    ...
    print(f"\n📊 Loading demonstration data: {dataset_path}")
    ...
    # ===============================
    output_path = '/home/seohy/colcon_ws/src/olaf/ffw/code/learned_mpc_weights.npz'
    ...
    print(f"\n💾 Saved learned weights: {output_path}")
...
"""
    ...
    for i, name in enumerate(['Q_pos', 'Q_vel', 'R_tau', 'Q_terminal']):
        ...
        print(f"   {name:12s}: {theta_init[i]:10.4f} → {theta_learned[i]:10.4f}  "
              f"({change:+6.1f}% {arrow})")
